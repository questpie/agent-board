...
- **Current Codex models in flows**: model discovery and flow execution now share the maintained `@agentclientprotocol/codex-acp` adapter (Codex CLI 0.144.4+), so authenticated Codex runtimes expose GPT-5.6 Sol/Terra/Luna instead of remaining pinned to the stale GPT-5.3-era adapter. `AGENT_BOARD_CODEX_ACP_BIN` overrides are honored consistently by both discovery and execution.
- **Editable design boards**: `agent-board wireframe scaffold` (alias `agent-board design new`) creates a zero-build HTML design board directly in the project store (`wireframes/<id>/`, git-versioned) from a shipped foundations → kit → screens template. Open it in `agent-board web` → Wireframes, toggle **Edit**, and a non-technical reviewer (or the agent) edits it in place — click any text to retype it, click any image to replace it, hover any auto-layout container to reorder it or adjust its spacing — and every change round-trips to the source HTML on disk. The board is the source of truth; the same static markup the agent authors is what a human edits, no separate design model.
- **Edits round-trip to static HTML, no database**: editing is a loopback-only `PUT /api/wireframe/edit` (the board's mutating routes stay localhost-gated, like share). Text and image edits are surgical — `HTMLRewriter` rewrites the addressed `[data-screen][data-key]` node in place for a minimal diff. Layout edits (reorder, `stack`↔`row` direction, and gap/padding stepping the `--space-*` token scale) swap a single screen's inner HTML with the browser as the DOM authority, and whitespace is reflowed so the diff stays just the change. All editing chrome lives in overlays and never lands in the saved markup.
- **Board media library**: an image can be swapped from the board's own asset store, an upload, or a URL. `GET /api/wireframe/assets` lists a board's media and a loopback `POST /api/wireframe/asset` saves an upload into the bundle's `assets/` (served and versioned with the board, image types only, size-capped).
- **Foundations → kit → screens discipline**: the scaffold ships a tiered token system (`tokens.css`: brand → semantic → scales, per-component `--c-*` vars, dark via `[data-theme]`), a token-driven component kit (`kit.css` plus a `components/` catalog of `@dsCard` cards — Button, Card, Field, List, Media, Layout, Typography, Foundations) and auto-layout primitives (`.stack`/`.row`/`.between`/`.grow`) — no raw hex or px outside `tokens.css`, every repeated element a reused class. The rewritten `agent-board-design-wireframe` skill enforces this (tokens-first, reuse-or-die, static granular `data-edit`/`data-key` leaves, auto-layout never absolute positioning) so what the agent authors stays consistent and live-editable. The generic shells (`board-stage.js`, `editor.js`, `preview.js`) are content-agnostic — HTML is the data, the shells are the runtime.
